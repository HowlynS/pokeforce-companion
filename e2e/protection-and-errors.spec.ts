...
test.describe("unauthenticated admin protection", () => {
  ...
  const PROTECTED_ROUTES = [
    ...
    // The Slice 9D.4 Metadata tab (read-only administrative information —
    // timestamps and verification — is still strictly admin-only); the
    ...
    // The Slice 9E.3 Items tab (read-only relationship content is still
    ...
    // navigation-foundation precedent.
    "/admin/locations/new",
    // The Slice 9F.3 Hierarchy tab (parent assignment plus read-only
    // sub-location content is still strictly admin-only); the auth gate
    // runs in the admin layout before the page's own database lookup, so
    // no seeded Location fixture is needed — this slug never resolves.
    "/admin/locations/test-e2e-missing-location/hierarchy",
    // The Slice 9F.4 Acquisition Sources tab (read-only relationship
    // content is still strictly admin-only); the auth gate runs in the
  ...
  for (const route of PROTECTED_ROUTES) {
    test(`${route} redirects to the login page`, async ({ page }) => {
      ...
      // The approved frontend proxy preserves the requested admin route so a
      // successful sign-in can return the user to the protected destination.

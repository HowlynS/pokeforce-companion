...
/**
 * Read-only availability probe for the Player Class name fields, invoked
 * from the client while typing. Admin authentication is enforced exactly
 * like every mutation: requireAdminUser() runs first and redirects any
 ...
 */
export async function checkPlayerClassNameAvailability(
  rawName: string,
  rawExcludeId?: string
  ...
  const taken = await isPlayerClassNameTaken(
    prisma,
    name,
    excludeId || undefined
  );

...
/**
 * Read-only availability probe for the Profession name fields, invoked from
 ...
 */
export async function checkProfessionNameAvailability(
  rawName: string,
  rawExcludeId?: string
  ...
  const taken = await isProfessionNameTaken(
    prisma,
    name,
    excludeId || undefined
  );
